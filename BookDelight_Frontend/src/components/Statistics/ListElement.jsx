    ...
    return (
        <div
            ...
            <div className="text-xl text-blue-500 font-semibold mb-4">
                <p className="text-lg lg:text-xl">{cardTitle}</p>
            </div>
            {items.map((item, index) => (
                <div key={index} className="place-items-center">
                    <div className="flex flex-row w-11/12 mb-1 last:mb-0">
                        <p className="text-base lg:text-lg mb-2 font-semibold">{item.text}: </p>
                        <p className="text-lg lg:text-xl ml-2 text-blue-500 font-semibold">{item.userData}</p>
                    </div>
                </div>
            ))}
        </div>
